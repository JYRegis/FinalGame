...
// 每帧调用
void ASheeeps::Tick(float DeltaTime)
{
...
// 启动移动功能
void ASheeeps::StartMovement()
{
	// 设置定时器，用于间隔调用 MoveToTarget 函数
	GetWorldTimerManager().SetTimer(MovementTimerHandle, this, &ASheeeps::MoveToTarget, 0.1f, true);
}

// 设置目标位置
void ASheeeps::SetTargetPosition(FVector NewTargetPosition)
{
	TargetPosition = NewTargetPosition;
	UE_LOG(LogTemp, Log, TEXT("Sheeeps SetTargetPosition: New target position set to (%f, %f, %f)."),
		TargetPosition.X, TargetPosition.Y, TargetPosition.Z);
}

// 自定义事件：开始移动
void ASheeeps::BeginCustomMovement()
{
	// 获取玩家的Pawn
...
// 随机生成目标点
FVector ASheeeps::GenerateRandomTargetLocation()
{
	// 生成随机目标点
